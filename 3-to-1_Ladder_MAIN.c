...
void floatToArray(float32 n, Uint16 *res, Uint16 afterpoint){
    Uint16 ipart = (Uint16)n;                   // Extract integer part
    float32 fpart = n - (float)ipart;           // Extract floating part
    Uint16 i = intToStr(ipart, res, 1);         // convert integer part to string
...
interrupt void adca0_isr(void){
    ...
    //this mode is the normal operation mode and can be used for efficiency measurements
        ...
        if((stabilityStartMeasure ==  1) & (stabilityCounter < stabilityBufferSize)){
            ...
            stabilityBuffer2[stabilityCounter] = AdcbResultRegs.ADCRESULT0;
            stabilityCounter++;
        }
    #endif
    ...
    //this mode is used to measure the transfer function of the converter without controller
    #ifdef OPEN_LOOP_PERTURBATION_MODE
        sgen.calc(&sgen);                               //calculate sine
        phiTemp = sgen.out/3276.7;                      //sgen.out max = 32767, scale output: 0 to 10
        ...
        //as soon as serial command set "stabilityStartMeasure=1" the measurement begins
        if((stabilityStartMeasure ==  1) & (stabilityCounter < stabilityBufferSize)){
            stabilityBuffer1[stabilityCounter] = phiTemp;
            stabilityBuffer2[stabilityCounter] = AdcbResultRegs.ADCRESULT0;
            stabilityCounter++;
        }
...
void HRPWM3_Config(period){
    ...
    EPwm3Regs.HRCNFG.bit.EDGMODE = HR_BEP;                      // MEP control of both edges (TBPHSHR or TBPRDHR)
    EPwm3Regs.HRCNFG.bit.EDGMODEB = HR_BEP;                     // MEP control of both edges (TBPHSHR or TBPRDHR)
